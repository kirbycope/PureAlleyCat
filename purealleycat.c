/* purealleycat - see purealleycat.h.
 *
 * A port of tools/emu8086.py, which was validated by running the real CAT.EXE to its attract
 * screen and decoding the framebuffer. The semantics here follow that implementation exactly;
 * where the two disagree, the Python one is the reference and this is wrong.
 */
...
static uint8_t  FRAME[PURECAT_WIDTH * PURECAT_HEIGHT];
...
int purecat_init(const void *exe_bytes, int exe_size)
{
...
void purecat_update(void) { purecat_run(TICK_STEPS); }
    ...
    for (y = 0; y < PURECAT_HEIGHT; y++) {
        ...
        for (xb = 0; xb < 80; xb++) {
            ...
            for (pair = 0; pair < 4; pair++)
                FRAME[y * PURECAT_WIDTH + xb * 4 + pair] = (uint8_t)((byte >> (6 - pair * 2)) & 3);
        }
    }
    return FRAME;
}
...
int purecat_ready(void) { return VIDEO_MODE >= 0; }
uint64_t purecat_instructions(void) { return ICOUNT; }
const char *purecat_status(void) { return STATUS; }
int purecat_fault_opcode(void) { return BAD_OP; }
uint32_t purecat_fault_address(void) { return ((uint32_t)BAD_CS << 16) | BAD_IP; }
